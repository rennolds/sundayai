<script>
  let { isLoading, results, onBackClick, pendingItems } = $props();
  ...
  // Check if a specific item is still being generated
  function isItemPending(type) {
    return pendingItems && pendingItems.includes(type);
  }
</script>
...
<div class="bg-white rounded-lg shadow-md">
  {#if isLoading && Object.keys(results).length === 0}
    <div class="flex flex-col items-center justify-center py-12 p-8">
      ...
      <p class="text-gray-500">This may take a minute or two. Items will appear as they're completed.</p>
    </div>
  {:else}
    ...
    <div class="flex overflow-x-auto border-b border-gray-200">
      {#each Object.keys(results) as key}
        ...
        >
          ...
          {/if}
          {getDisplayName(key)}
          {#if pendingItems && isItemPending(key)}
            <div class="ml-2 w-4 h-4 rounded-full border-2 border-blue-500 border-t-transparent animate-spin"></div>
          {/if}
        </button>
      {/each}
      ...
      {#if pendingItems && pendingItems.length > 0}
        {#each pendingItems as pendingItem}
          {#if !Object.keys(results).includes(pendingItem)}
            <button
              class="flex items-center px-4 py-3 font-medium text-sm text-gray-500 focus:outline-none"
              disabled={true}
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
              </svg>
              {getDisplayName(pendingItem)}
              <div class="ml-2 w-4 h-4 rounded-full border-2 border-blue-500 border-t-transparent animate-spin"></div>
            </button>
          {/if}
        {/each}
    ...
    <!-- Status bar for pending items if any are still generating -->
    {#if isLoading && pendingItems && pendingItems.length > 0}
      <div class="border-t border-gray-200 px-6 py-3 bg-blue-50">
        <div class="flex items-center text-blue-700">
          <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-blue-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <span>Generating {pendingItems.length} more item{pendingItems.length > 1 ? 's' : ''}... You can view completed items while waiting.</span>
        </div>
      </div>
    {/if}
